...
namespace natural
{
	...
	************************************************************************/
	...
	bool _compare(\
		...
		const Iterator &rhsBegin,const Iterator &rhsEnd)
	{
		return _comparei<ElementType, Iterator>(lhsBegin, lhsEnd, rhsBegin, rhsEnd) < 0;
	}

	template<typename String>
	inline int comparei(const String &first ,const String &second)
	{
		return _comparei<typename String::value_type,typename String::const_iterator>(first.begin(),first.end(),second.begin(),second.end());
	}
